...
A provider is `{label, env, claudeJson?, modelCatalog?}`. Adding a simple provider is a pure data change. `modelCatalog` is what unlocks the model-selection flow: it names the models/endpoints/presets URLs (live, OpenRouter-shaped) or a static `models` list (id/name/context_length/pricing records served offline — the bundled Z.ai GLM entry, since Z.ai publishes no models API) plus the env keys `m` is allowed to write (`modelEnv`, `selectionEnv`, `contextEnv` — optionally including an `autoCompact` key written with the same selected window — `recognitionOverride`, `specialModels`, `excludeIdSuffixes` — catalog variants hidden from the picker and rejected by `model_exists`, bundled `[":batch"]`). Code reads these key *names* from JSON rather than hardcoding them, so a second live catalog provider is expressible in `providers.json` alone *if* it speaks OpenRouter's API shapes (`.data[]` models, `.data.endpoints[]`, presets at `<presetsUrl>/<slug>[/messages]`), and a first-party gateway needs only the static list.
...
- **Subagents are forced onto the selected model.** Catalog providers' `env` blocks carry `CLAUDE_CODE_SUBAGENT_MODEL_FORCE=1` because, since Claude Code 2.1.251, an agent definition's `model:` or a per-spawn model beats `CLAUDE_CODE_SUBAGENT_MODEL` (a plugin agent pinned to a Claude ID would leave the route). Tests assert the flag after every model switch and its absence after the round trip; keep it an `env` key so strip-then-merge removes it.
...
`catalog_switch()` is the spine: `ensure_credential` → `fetch_models` → `model_picker` → `fetch_endpoints` → `endpoint_picker` → `ensure_routing_preset` → `switch_to`. Both pickers fall back to non-interactive behavior when stdin/stdout are not a TTY (exact model required; cheapest healthy tool-capable endpoint auto-selected). Providers with a static `models` catalog and no `endpointsUrl` (Z.ai) take the direct branch instead — no endpoints, no preset, `switch_to` with the model ID itself — and `fetch_models` builds their catalog offline, so no request or credential is involved in browsing.
